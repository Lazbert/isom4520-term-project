...
from .conduct_univariate_multivariate_hyp_testing import (
...
def research(**params):
    ...
    df = pd.read_csv(
        os.path.join(enums.POTENTIAL_TRADES_DIR, f"{cache_key}.csv"), index_col=0
    )
    df = convert_time_columns_to_datetime(df, params["market"])

    df_original = pd.read_csv(
        os.path.join(enums.POTENTIAL_TRADES_DIR, f"{cache_key}-Original.csv"),
        index_col=0,
    )
